...
In similar way to the RTL implementation, please set `implmentation="naive"` or `implmentation="rsm"` keyword argument to the `con` method or `cw.target` routine.
...
# if you want to use cw.target routine, use the following line
# cw.target(scope, SakuraXShellExampleAES128BitRTL, hwh_file=hwh_file)
