...
### Examples
That repository contains implementations for some example designs.
They integate each core in [sca_design_repo](https://github.com/hal-lab-u-tokyo/sca_design_repo) with the shell template.
...
To specify the implementation, please set `implmentation="aist"` or `implmentation="google"` keyword argument to the `con` method.
...
If you want to use your own hardware handoff file, please set `hwh_file` keyword argument to the path of the hardware handoff file when calling `con` method.
