...
class DHO900(ScopeBase):
    """
        Rigol DHO900 series oscilloscope class
    """
    slope_str = {
    ...
    mem_depth_options = [
        # 1k, 10k, 100k, 1M, 10M, 25M, 50M
        # in 2ch mode, upper limit is 25M
        # in 4ch mode, upper limit is 10M
        1e+3, 10e+3, 100e+3, 1e+6, 10e+6, 25e+6, 50e+6
    ]
    CHUNK_SIZE = 1000
    def __init__(self, model, resource, timeout):
        super().__init__(resource, timeout)
        self.max_sampling_rate = 625e+6 # 625MS/s
        self.max_mem_depth = 25e+6 # 25Mpts
        self.start_pos = 1
        self.sample_count = 1
    ...
    def config_trace_channel(self, channel, scale, offset, period, delay = 0):
